<template>
    <div class="container">
        <div class="card-header px-0 mt-2 bg-transparent clearfix">
            <h4 class="float-left pt-2">Personal Administrativo</h4>
            <div class="card-header-actions mr-1">
                <a class="btn btn-success" :href="route('users.ad.add')" title="Añadir usuario">
        ...
        <div class="card-body px-0">
                ...
                <table class="table table-hover table-outline">
                    ...
                    <tbody>
                        <tr v-for="user in users" :key="user.id">
                            ...
                            <td><a class="text-dark" :href="route('users.ad.show', user.id)" :title="'Ver más datos de '+user.first_name+' '+user.last_name">{{user.first_name+' '+user.last_name}}</a></td>
            ...
            <div class="no-items-found text-center mt-5" v-if="!loading && !users.length > 0">
                ...
                <a class="btn btn-success" :href="route('users.ad.add')" role="button">
...
<script>
    export default {
        ...
        mounted() {
            if (localStorage.getItem("filtersTableUsersAD")) {
                this.filters = JSON.parse(localStorage.getItem("filtersTableUsersAD"));
            } else {
                localStorage.setItem("filtersTableUsersAD", JSON.stringify(this.filters));
            }
            this.getUsers();
        },
        methods: {
                ...
                if (!this.submiting) {
                    ...
                    if(this.usersids.length == 0){
                        ...
                        if(action.value == 'delete'){
                            ...
                            .then((willDelete) => {
                                if (willDelete) {
                                    axios.post(route('api.users.ad.masschanges'), {
                                        users: this.usersids,
                                        action: this.action
                                    })
                                    .then(response => {
            ...
            getUsers(page = 1) {
                ...
                this.status = null;
                localStorage.setItem("filtersTableUsersAD", JSON.stringify(this.filters));
                axios.post(route('api.users.ad.index'), this.filters, {params: {page: page}})
                .then(response => {
            ...
            editUser (userId) {
                location.href = route('users.ad.edit', userId);
            },
            showUser (userId) {
                location.href = route('users.ad.show', userId);
            },
            changeSize (perPage) {
                this.filters.pagination.current_page = 1
            ...
            deleteUser(userid) {
                if (!this.submiting) {
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                axios.post(route('api.users.ad.destroy', userid))
                                .then(response => {
                                    this.getUsers();
                                    this.submiting = false;
                                    this.$toasted.global.error(response.data.message);
