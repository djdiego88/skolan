...
<div class="user-buttons clearfix">
	<a href="{{route('users.ad.edit', $user->id)}}" class="btn btn-secondary active btn-sm"><i class="fas fa-edit"></i> Actualizar a {{$name}}</a>
	<a href="{{route('users.ad.destroy', $user->id)}}" onclick="return confirm('¿Deseas eliminar este Super Administrador?');" class="btn btn-secondary active btn-sm"><i class="fas fa-times"></i> Eliminar a {{$name}}</a>
</div>
