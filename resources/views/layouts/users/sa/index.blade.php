...
<div class="tablenav clearfix">
	{!! Form::open(['route' => 'users.sa.index', 'method' => 'GET', 'class' => 'float-sm-right']); !!}
...
{!! Form::open(['route' => 'users.sa.masschanges', 'method' => 'POST']); !!}
	<div class="row">
		<div class="col-sm-3">
			<div class="input-group">
		...
		<div class="col-sm-3 col-sm-offset-3">
			<div class="text-xs-right">{{count($users)}} elementos</div>
		</div>
	</div>
		...
		<table class="table table-hover table-bordered table-sm">
			...
			?>
				<tr>
					...
					<td><a href="{{route('users.sa.show', $user->id)}}" title="Ver más datos de {{$name}}">{{$name}}</a></td>
					...
					<td>
					@if($user->id != 1)
						<a href="{{route('users.sa.edit', $user->id)}}" class="btn btn-sm btn-warning" title="Editar datos de {{$name}}"><i class="fas fa-edit"></i></a><a href="{{route('users.sa.destroy', $user->id)}}" onclick="return confirm('¿Deseas eliminar este Super Administrador?');" class="btn btn-sm btn-danger" title="Eliminar a {{$name}}"><i class="fas fa-times"></i></a>
					@endif
					</td>
				</tr>
			@endforeach	
